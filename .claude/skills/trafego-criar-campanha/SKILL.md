...
A especificação técnica completa está em `.claude/skills/trafego-criar-campanha/spec.md`.
...
Leia `memoria/produto.md` para inferir produto, ticket, posicionamento e público quando
...
- **Se vazio ou ausente:** acione `/trafego-conexao` antes de prosseguir.
- **Se `MCP_CONECTOR`:** confirmar que pelo menos uma tool `mcp__*__ads_*` está disponível.
  ...
  Se persistir, voltar a `/trafego-conexao`.
- **Se `APP`:** confirmar que `FB_ACCESS_TOKEN_PERMANENTE`, `FB_AD_ACCOUNT_ID` e `FB_PAGE_ID`
  existem no `.env`. Se faltar algum, acionar `/trafego-conexao`.
...
Leia `.claude/skills/trafego-criar-campanha/spec.md` para carregar fluxo de coleta,
...
2. **Produto e ticket**. Infere de `memoria/produto.md` se possível, senão pergunta.
...
Salvar preview em: `08-trafego-meta/preview-campanha-{tipo_funil}-{slug}-{YYYY-MM-DD}.yaml`
...
Salvar resumo em:
```
08-trafego-meta/campanha-{nome-slug}-{YYYY-MM-DD}.md
```

...
description: >-
  ...
  de marca de memoria/design.md). Também dispara com "/brand-carrossel", "carrossel editorial
  com marca", "carrossel Principal" ou qualquer pedido de carrossel que use a marca do projeto.
---


> **Marca = carregada de `memoria/design.md`.** HANDLE, TAGLINE, COPYRIGHT, LOGO e
> ASSINATURA vêm da memória do projeto — nunca inventar, nunca perguntar ao usuário.
> Se `memoria/design.md` estiver ausente ou com `status: vazio`, **PARE** e peça `/instalar`.

> **Como usar:** ao invocar `/brand-carrossel`, forneça apenas TEMA, ÂNGULO e (opcionalmente)
...
0. **Carregar memória de marca** — ler `memoria/design.md` e extrair:
   HANDLE, TAGLINE, COPYRIGHT, LOGO (caminho do arquivo), ASSINATURA.
   Se ausente ou `status: vazio` → **PARAR. Executar `/instalar` antes.**
...
4. **Montar o HTML** a partir do scaffold (Seção 5) + colar os recipes (Seção 6),
   substituindo todos os `{{PLACEHOLDER}}` com os dados reais de `memoria/design.md`.
...
O usuário fornece **apenas o conteúdo editorial**. Dados de marca vêm de `memoria/design.md`
...
```
/carrossel-<slug>.html      → todos os slides (este é o entregável)
